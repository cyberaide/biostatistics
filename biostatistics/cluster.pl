...
#    for $VOL_PARAM (@volume_inc_set) {
	...
	$command = "cd $ARGV[13]/NVIDIA_CUDA_SDK/projects/biostatistics/; ./script.sh $CLUSTER_NUM $NUM_DIMENSIONS $NUM_EVENTS $ARGV[2] $ARGV[3] $ARGV[4] $ARGV[5] $ARGV[6] $ARGV[7] $ARGV[8] $ARGV[9] $ARGV[10] $ARGV[11]  $ARGV[12] $NUM_THREADS; cd -";
	printf "$command\n";
	system $command;
	$compile = "make clean -C $ARGV[13]/NVIDIA_CUDA_SDK/projects/biostatistics/; make -C $ARGV[13]/NVIDIA_CUDA_SDK/projects/biostatistics/";
	printf "$compile\n";    
	system $compile;
	$run= "$ARGV[13]/NVIDIA_CUDA_SDK/bin/linux/release/cmeans $fn";
	printf "$run\n";
	system $run;
#    }
}
