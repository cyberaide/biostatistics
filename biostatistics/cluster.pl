...
if($fn =~ m/fcs$/)
{
	...
	#$newfn =~ s/fcs/txt/;
	$newfn = $newfn . ".dat";
	$rcommand = "cd $ARGV[13]/projects/biostatistics/R/; R --no-save < convertFCS.r $fn $newfn; cd -";
	printf "$rcommand";
	system $rcommand;
	$fn = $newfn;
}
...
my(@lines) = <MYFCSFILE>;
my $tmp = `grep "DELIMITER" ~/NVIDIA_CUDA_SDK/projects/biostatistics/cmeans.h`;
printf "tmp = $tmp\n";
my(@temps) = split(/ /, $tmp);
printf "temps = @temps\n";
my $delimiter = @temps[2];
printf "delimiter = $delimiter\n";
my(@tokens) = split /$delimiter/,@lines[0];
...
#    for $VOL_PARAM (@volume_inc_set) {
	...
	$command = "cd $ARGV[13]/projects/biostatistics/; ./script.sh $CLUSTER_NUM $NUM_DIMENSIONS $NUM_EVENTS $ARGV[2] $ARGV[3] $ARGV[4] $ARGV[5] $ARGV[6] $ARGV[7] $ARGV[8] $ARGV[9] $ARGV[10] $ARGV[11]  $ARGV[12] $NUM_THREADS; cd -";
	printf "$command\n";
	system $command;
	$compile = "make clean -C $ARGV[13]/projects/biostatistics/; make -C $ARGV[13]/projects/biostatistics/";
	printf "$compile\n";    
	system $compile;
