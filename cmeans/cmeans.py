...
PROJECT_PATH = os.path.abspath(os.path.expanduser("~/NVIDIA_GPU_Computing_SDK/C/src/cmeans"))

# Delimiter used in the input files
DELIMITER = ","

def usage():
    print "Usage: ./cmeans.py INPUT_FILE NUM_CLUSTERS [THRESHOLD=0.0001] [MIN_ITERS=0] [MAX_ITERS=100] [DEVICE=0] [FUZZINESS=2] [DISTANCE_MEASURE=0] [K1=1.0] [K2=0.01] [K3=1.5] [MEMBER_THRESHOLD=0.05] [TABU_ITER=100] [TABU_TENURE=5] [MDL=0] [CPU_ONLY=0] [RANDOM_SEED=1] [TRUNCATE=1]"
    print
...
def parseInputArgs():
    ...
    if num_args == num_params:
        ...
        params['RANDOM_SEED'] = args[17]
        params['TRUNCATE'] = args[18]
    elif num_args == 3:
        params['INPUT_FILE'] = args[1]
        params['NUM_CLUSTERS'] = args[2]
...
if __name__ == '__main__':
    ...
    if not error:
        cmd = '%s/../../bin/linux/release/cmeansSingleGPU "%s"' % (PROJECT_PATH,params['INPUT_FILE'])
        print cmd
        os.system(cmd)
